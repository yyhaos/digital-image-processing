...
BEGIN_MESSAGE_MAP(Cbasic_imreadDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BUTTON1, &Cbasic_imreadDlg::OnBnClickedButton1)
END_MESSAGE_MAP()
...
void Cbasic_imreadDlg::OnBnClickedButton1() // 显示直方图按钮
{
	if (path == "")
	{
		MessageBox(TEXT("必须先打开bmp图片"), TEXT("错误"), MB_OK);
	...
	ttr.open(path);
	ttr.init_2();
	//ttr.print_histogram(m_pTipDlg, IDD_DIALOG1);
	m_pTipDlg = new Histogram_show(ttr.bi );
